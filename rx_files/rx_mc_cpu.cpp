 ...
 */
bool rxMCMeshCPU::CreateMesh(RXREAL (*func)(double, double, double, void*), void* func_ptr, Vec3 min_p, double h, int n[3], RXREAL threshold, 
							 vector<Vec3> &vrts, vector<Vec3> &nrms, vector<rxFace> &face)
{
	...
	vector<int> tris;
	GenerateSurfaceV(sf, func, func_ptr, threshold, vrts, nrms, tris);
 ...
 * @param[out] tris メッシュ幾何情報(頂点接続情報)
 */
void rxMCMeshCPU::GenerateSurfaceV(const RxScalarField sf, RXREAL (*func)(double, double, double, void*), void* func_ptr, RXREAL threshold, 
										  vector<Vec3> &vrts, vector<Vec3> &nrms, vector<int> &tris)
{
	int nx, ny, nz;
	nx = sf.iNum[0]+1;
	...
	for(int k = 0; k < nz; ++k){
		for(int j = 0; j < ny; ++j){
			for(int i = 0; i < nx; ++i){
				...
				RXREAL val = func(pos[0], pos[1], pos[2], func_ptr);
				field[idx] = val;
			}
		}
	}
 ...
 * @param[out] tris メッシュ幾何情報(頂点接続情報)
 */
void rxMCMeshCPU::GenerateSurfaceF(const RxScalarField sf, RXREAL (*func)(double, double, double, void*), void* func_ptr, RXREAL threshold, 
										  vector<Vec3> &vrts, vector<Vec3> &nrms, vector<int> &tris)
{
	if(m_bValidSurface){
		DeleteSurface();
	...
	for(uint k = 0; k < m_Grid.iNum[2]; ++k){
		for(uint j = 0; j < m_Grid.iNum[1]; ++j){
			for(uint i = 0; i < m_Grid.iNum[0]; ++i){
				...
				// グリッド内の頂点配置情報テーブル参照用インデックスの計算
				uint tableIndex = 0;
				if(m_fpScalarFunc(x,    y,    z, m_pScalarFuncPtr) < m_tIsoLevel) tableIndex |= 1;
				if(m_fpScalarFunc(x,    y+dy, z, m_pScalarFuncPtr) < m_tIsoLevel) tableIndex |= 2;
				if(m_fpScalarFunc(x+dx, y+dy, z, m_pScalarFuncPtr) < m_tIsoLevel) tableIndex |= 4;
				if(m_fpScalarFunc(x+dx, y,    z, m_pScalarFuncPtr) < m_tIsoLevel) tableIndex |= 8;
				if(m_fpScalarFunc(x,    y,    z+dz, m_pScalarFuncPtr) < m_tIsoLevel) tableIndex |= 16;
				if(m_fpScalarFunc(x,    y+dy, z+dz, m_pScalarFuncPtr) < m_tIsoLevel) tableIndex |= 32;
				if(m_fpScalarFunc(x+dx, y+dy, z+dz, m_pScalarFuncPtr) < m_tIsoLevel) tableIndex |= 64;
				if(m_fpScalarFunc(x+dx, y,    z+dz, m_pScalarFuncPtr) < m_tIsoLevel) tableIndex |= 128;
...
RxVertexID rxMCMeshCPU::CalculateIntersectionF(uint nX, uint nY, uint nZ, uint nEdgeNo)
{
	...
	RXREAL val1 = m_fpScalarFunc(x1, y1, z1, m_pScalarFuncPtr);
	RXREAL val2 = m_fpScalarFunc(x2, y2, z2, m_pScalarFuncPtr);
	RxVertexID intersection = Interpolate(x1, y1, z1, x2, y2, z2, val1, val2);
